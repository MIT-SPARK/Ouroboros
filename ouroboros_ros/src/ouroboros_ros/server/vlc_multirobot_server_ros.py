...
class VlcMultirobotServerRos(VlcServerRos):
    def __init__(self, node_name="vlc_server_node"):
        ...
        self.robot_infos = {}
        self.uuid_map = {}
        self.session_robot_map = {self.session_id: self.robot_id}
    ...
    def info_timer_callback(self):
        ...
        self.robot_infos[self.robot_id] = RobotInfo.from_info_msg(info_msg)
    ...
    def process_new_frame(self, image, stamp_ns, hint_pose):
        new_uuid = super().process_new_frame(image, stamp_ns, hint_pose)
    ...
    def client_embedding_callback(self, vlc_img_msg, robot_id):
        ...
        with self.candidates_lock:
            self.candidates.append((new_uuid, matched_img.metadata.image_uuid))
    ...
    def publish_lcs(self, loop_closures):
        ...
        for lc in loop_closures:
            ...
            from_session_id, to_session_id = self.vlc_server.get_lc_sessions(
                lc.metadata.lc_uuid
            )

            from_robot = self.session_robot_map[from_session_id]
            to_robot = self.session_robot_map[to_session_id]
            lc_edge = build_lc_message(
                ...
                lc.f_T_t,
                pose_cov_mat,
                self.robot_infos[from_robot].body_T_cam.as_matrix(),
                self.robot_infos[to_robot].body_T_cam.as_matrix(),
                self.get_clock().now(),
            )
    ...
    def request_keypoints(self, image_uuid, robot_id):
        # Request keypoints / descriptors
        keypoints_request = VlcKeypointQuery.Request()
        # Request using the correct image uuid (kept in the uuid_map)
        keypoints_request.image_uuid = self.uuid_map[image_uuid]
        future = self.keypoint_clients[robot_id].call_async(keypoints_request)
        future.add_done_callback(
            lambda f: self.handle_keypoints_response(f, image_uuid)
        )

    def handle_keypoints_response(self, future, image_uuid):
        result = future.result()
        if result is None:
            self.get_logger().error("Keypoints service call failed to return")
            return

        vlc_response = vlc_image_from_msg(future.result().vlc_image)
        self.vlc_server.update_keypoints_decriptors(
            image_uuid, vlc_response.keypoints, vlc_response.descriptors
        )
        self.vlc_server.update_keypoint_depths(image_uuid, vlc_response.keypoint_depths)

    def compute_keypoints(self):
        with self.candidates_lock:
            for query_uuid, match_uuid in self.candidates:
                query_image = self.vlc_server.get_image(query_uuid)
                if query_image.keypoints is None:
                    if query_image.metadata.session_id == self.session_id:
                        # keypoints on this server
                        self.vlc_server.compute_keypoints_descriptors(
                            query_uuid, compute_depths=True
                        )
                    else:
                        # request
                        robot_id = self.session_robot_map[
                            query_image.metadata.session_id
                        ]
                        self.request_keypoints(query_uuid, robot_id)

                match_image = self.vlc_server.get_image(match_uuid)
                if match_image.keypoints is None:
                    if match_image.metadata.session_id == self.session_id:
                        # keypoints on this server
                        self.vlc_server.compute_keypoints_descriptors(
                        ...
                        # request
                        robot_id = self.session_robot_map[
                            match_image.metadata.session_id
                        ]
                        self.request_keypoints(match_uuid, robot_id)
